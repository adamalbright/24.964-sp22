...
$constraintsfile = $ARGV[1];
while (!$valid_constraintsfile) {
	if ($constraintsfile eq "") {
		$constraintsfile = $inputfile;
		# First, strip of the "extension" (.txt, etc.)
		$constraintsfile =~ s/\.[^\.]*$//;
		$constraintsfile .= ".constraints";
	}
	if (-e $constraintsfile) {
		$valid_constraintsfile = 1;
	} else {
...
printf RANKINGSFILE "Time\t". join("\t",@constraintnames)."\n";
...
for (my $t = 1; $t <= $number_of_learning_trials; $t++) {
	...
	for (my $c = 0; $c <= $number_of_constraints; $c++) {
		...
		# adding evaluation noise:   (does it actually matter to add evaluation noise during training?)
		$actual_ranking_value[$c] = $ranking_value[$c] + evaluation_noise();		
	}
...
for (my $i = 1; $i <= $number_of_inputs; $i++) {
	...
	for (1 .. $number_of_testing_trials) {
		# Jitter the constraints according to the evaluation noise
		for (my $c = 0; $c <= $number_of_constraints; $c++) {
			$current_total_hierarchy[$c] = $c;
			# adding evaluation noise:   
			$actual_ranking_value[$c] = $ranking_value[$c] + evaluation_noise();
		}
		@current_total_hierarchy = sort { $actual_ranking_value[$b] <=> $actual_ranking_value[$a] } @current_total_hierarchy;
...
sub evaluation_noise {
    ...
    # Scale by evaluation noise, by multiplying by sqrt
    return sqrt($evaluation_noise) * $g1;
}
